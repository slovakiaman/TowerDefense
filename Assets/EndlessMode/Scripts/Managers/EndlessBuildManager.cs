...
public class EndlessBuildManager : MonoBehaviour
{
    public static EndlessBuildManager instance;

    private Vector3 positionOffset = new Vector3(0f, 2f, 0f);

    private EndlessBuildingBlock selectedBlock;

    private bool disabled = false;

    private EndlessShop shop;
    ...
    private void Start()
    {
        shop = EndlessUIManager.instance.GetShop();
    }

    public void SetShop(EndlessShop shop)
    {
        this.shop = shop;
    }

    public EndlessBuildingBlock GetSelectedBlock()
    {
        return selectedBlock;
    }

    public void SetSelectedBlock(EndlessBuildingBlock block)
    {
        if (disabled)
            return;
    ...
    {
        GameObject upgradeTower = tower.GetComponent<EndlessTower>().GetUpgradeTower();
        if (upgradeTower != null)
        {
            DestroyTower(selectedBlock.GetTower());
            selectedBlock.SetTower(null);
            BuildTower(tower.GetComponent<EndlessTower>().GetUpgradeTower());
        }
    }
